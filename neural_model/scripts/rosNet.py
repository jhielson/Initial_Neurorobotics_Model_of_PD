...
def talker(countSpikes):
    print("NetPyNe to ROS")
    pub = rospy.Publisher('spikes', String, queue_size=10) 
    send_str = "%s" % countSpikes
    rospy.loginfo(send_str)
    pub.publish(send_str)

if __name__ == '__main__':
   
    rospy.init_node('netpyne_jh', anonymous=True)
	...
	## Population parameters
        netParams.popParams['S'] = {'cellType': 'PYR', 'numCells': 20, 'cellModel': 'Izhi'}         #'Izhi'} 
	...
	# Count spikes
        countSpikes = 0
        cell = 36
        for i in range(len(sim.allSimData['spkid'])):
            if cell == sim.allSimData['spkid'][i]:
                countSpikes += 1
                print('Time: %s' % sim.allSimData['spkt'][i])

        print('Total: ' + str(countSpikes))

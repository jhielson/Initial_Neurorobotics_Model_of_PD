...
if __name__ == '__main__':
    try:
        ...
        # Thalamic current
        current = 0.0012 #0.0012
        
        # Healthy
        pd = 0
        data = np.empty(shape=[0, 3]) 
        for exp in range(1,2):
            network1 = Network(th = current, t_sim = 90000, has_pd=pd)
        ...
        #plt.plot(network1.data_X_H,network1.data_Y_H, label='Healthy State')
        #plt.plot(network2.data_X_PD,network2.data_Y_PD, label='Parkinsonian State')
        #plt.title('PSD (0.0035mA)')

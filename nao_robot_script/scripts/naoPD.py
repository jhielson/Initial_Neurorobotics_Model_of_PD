...
class Robot(object):
    def __init__(self):
        ...
        #Sub
        rospy.Subscriber('ball_colour', String, self.callback_colour)
        ...
        rospy.Subscriber('perturbation',Float64,self.callback_perturbation)
        ...
        while not rospy.is_shutdown():
            ...
            if self.state == 0:
                ...
                if self.flag_stimuli:
                    ...
                    #Move arm
                    self.joint_names = ['RShoulderPitch','RShoulderRoll','RWristYaw','RElbowRoll','RElbowYaw','RHand']
                    self.joint_angles = [0.61,-0.05+self.noise,1.47+self.noise,0.96+self.noise,1.69+self.noise,0.5+self.noise]
                    if self.counter_wait >= 80:
                        self.state = 3
                        self.speak_once = False
                        self.counter_wait = 0
                    else:
                ...
                self.joint_names = ['RShoulderPitch','RShoulderRoll','RWristYaw','RElbowRoll','RElbowYaw','RHand']
                self.joint_angles = [0.61,0.10+self.noise,1.44+self.noise,0.83+self.noise,0.25+self.noise,0.6+self.noise]
                if self.counter_wait >= 80:
                    if count_trials == self.trials:
                        self.state = 4
                    else:
                ...
                #Move right arm to the rest pose
                msg = JointAnglesWithSpeed()
                msg.joint_names = ['RShoulderPitch','RShoulderRoll','RWristYaw','RElbowRoll','RElbowYaw','RHand']
                msg.joint_angles = [0.61,-0.55+self.noise,1.44+self.noise,0.83+self.noise,0.25+self.noise,0.0+self.noise]
                msg.speed = 0.1
                msg.relative = 0
                self.pub_joints.publish(msg)
                time.sleep(1)
                #Move arm to the rest pose
                msg = JointAnglesWithSpeed()
                msg.joint_names = ['LShoulderPitch','LShoulderRoll','LWristYaw','LElbowRoll','LElbowYaw']
                msg.joint_angles = [1.35,1.00,-0.53,-1.10,-1.24]
                msg.speed = 0.1
